...
def load_s2_stack(
    ...
    *,
    ...
    ``fail_on_error=False`` — не небрежность, а условие работоспособности.
    В каталоге встречаются элементы, у которых метаданные есть, а ассеты
    отдают 404: снимок отозван, переложен или ещё не доехал до хранилища.
    По умолчанию odc-stac роняет всю загрузку на первом таком файле, и один
    испорченный снимок из восьмисот убивает плитку целиком.

    Так и случилось: сцена S2B от 12 июля 2026 года отдавала 404, и на ней
    падала КАЖДАЯ плитка кольца — обе попытки подряд, потому что повтор
    запрашивал ту же сцену. Прогон шёл сорок минут и не записал ни одного
    результата.

    Пропуск нечитаемого честнее падения: месячная медиана считается по
    десятку наблюдений, и потеря одного из них не меняет её. Потеря всех
    восьмисот меняет.
    """
