...
#:
#: 3 дБ — двукратное изменение мощности между проходами — это шкала для
#: пикселя, целиком занятого меняющейся поверхностью. Наши объекты мельче
#: разрешения RTC в 20 метров лишь немногим, и усреднение по контуру
#: подмешивает неподвижный фон.
#:
#: На кольцевом прогоне: медиана 0,20 дБ, девяностый процентиль 1,88 дБ,
#: максимум 2,57 дБ. Верхняя треть шкалы не достигалась никогда.
#:
#: 2.5 дБ — наблюдённый предел, округлённый вверх.
FULL_SCALE_INCOHERENCE_DB = 2.5
