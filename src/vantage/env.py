...
def configure(*, gdal_cache_mb: int = 512) -> None:
    ...
    # 404 в списке нет намеренно: отсутствующий файл не появится от повтора,
    # а восемь попыток по секунде на каждую из сотен ссылок съедают прогон.
    # Пропуск таких сцен делается заранее — StacCatalog._drop_broken.
    os.environ.setdefault("GDAL_HTTP_RETRY_CODES", "403,429,500,502,503,504")
