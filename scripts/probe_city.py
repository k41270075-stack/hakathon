...
def osm_density(bbox: tuple[float, float, float, float], settings) -> float:
    ...
    Замерено на семи областях:

        Алматы, первая        252,9 контура на км²  0 ОБЪЕКТОВ из 499 кандидатов
        запад · промзона       97,8                 41 объект из 552
        север Астаны           94,7                 3 подтверждённых + 5 опознанных
        Алматы, вторая         41,4                 0 настоящих из 8
        запад · сёла           32,5                 не запускался
        юго-восток             18,1                 0 настоящих из 9
        восток                  6,8                 0 настоящих из 33

    Показатель НЕ монотонный, и это выяснилось на Алматы. Провалы сверху и
    снизу — разные:

    **Снизу** карта пуста, отсеивать нечем, и до списка доходит всё подряд:
    восток дал 33 объекта, ни одного настоящего.

    **Сверху** карта настолько подробна, что отсев снимает всё: первая
    область Алматы дала ноль объектов из 499 кандидатов — каждый оказался
    либо слишком близко к жилью, либо на уже размеченной земле. Это плотная
    городская застройка, где свободного места просто нет.

    Годная полоса — примерно от 50 до 200. Обе границы стоят на одном
    наблюдении каждая, и потому названы приблизительными: это калибровка по
    семи прогонам, а не закон.
...
def verdict(share: float, industry: float | None = None,
    ...
    if density is not None and density > 200:
        return (f"НЕ ЗАПУСКАТЬ: сплошная застройка ({density:.0f} на км²) — "
                f"отсев снимет всё. Так выглядела первая область Алматы (253): "
                f"ноль объектов из 499 кандидатов")
    if density is not None and density < 50:
        return (f"неизвестно: карта средняя ({density:.0f} на км²) — здесь один "
                f"прогон, Алматы при 41: восемь находок, настоящих свалок ноль")
