...
── Что снимается ───────────────────────────────────────────────────────
    ...
    audience     кто пользуется: один главный пользователь и три контура
    economy      из чего складываются потери: вывоз − вторсырьё + климат
    decision     сравнение смет: вывезти как есть или с разбором
...
#: Разделы лендинга ищутся по идентификатору, а не по порядковому номеру.
#:
#: Прежде здесь стояло «main > section:nth-of-type(3)», и это ломалось от
#: любой вставки: за двое суток на лендинг добавились два раздела, номера
#: сдвинулись дважды, и набор для деки оба раза собирался молча и не тем.
#: Пропавшая картинка заметна сразу, подменившаяся — нет.
SHOTS: tuple[tuple[str, str, str], ...] = (
    ("pixel",    "index.html",    "#hero"),      # график вегетации
    ("signals",  "index.html",    "#signals"),   # пять признаков
    ("funnel",   "index.html",    "#funnel"),    # воронка отсева
    ("money",    "index.html",    "#when"),      # когда появились и сколько стоят
    ("scale",    "index.html",    "#scale"),     # измеренная скорость
    ("limits",   "index.html",    "#limits"),    # чего система не может
    ("audience", "index.html",    "#audience"),  # кто пользуется: один главный
    ("citizen",  "citizen.html",  "section:nth-of-type(1)"),
    # Экран экономики — тоже по идентификаторам. Порядковые номера здесь
    # уже успели соврать: между раскладкой и приоритетом вклинился раздел
    # «что выгоднее сделать», и снимок с именем priority.png стал снимать
    # сравнение смет. Ошибка тихая: файл на месте, картинка красивая, на
    # слайде не то.
    ("economy",  "economy.html",  "#breakdown"),   # из чего складывается сумма
    ("decision", "economy.html",  "#decision"),    # что выгоднее сделать
    ("priority", "economy.html",  "#priority"),    # очередь по деньгам
)
