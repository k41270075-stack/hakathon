...
def main() -> int:
    ...
    rejected = np.array(groups["Астана · отвергнутые"])

    log.info("")
    if len(dumps) < 2 or len(fields) < 2:
    ...
    # ── Ловушка, ради которой написана эта проверка ──────────────────
    #
    # Свалки взяты в Астане, пашня — в Алматы. Разделение между ними
    # может означать не «свалка против пашни», а «Астана против Алматы»:
    # разный климат, разная длина сезона, разное орошение.
    #
    # Отличить одно от другого помогает третья группа — отвергнутые
    # объекты Астаны. Это НЕ свалки, и лежат они в том же городе, что и
    # свалки. Если признак про класс, они должны быть похожи на пашню.
    # Если про местность — на свалки.
    log.info("Диапазоны НЕ перекрываются: свалки %.3f–%.3f, пашня %.3f–%.3f.",
             dumps.min(), dumps.max(), fields.min(), fields.max())

    if len(rejected) >= 3:
        inside_dumps = ((rejected >= dumps.min()) & (rejected <= dumps.max())).mean()
        inside_fields = ((rejected >= fields.min()) & (rejected <= fields.max())).mean()
        log.info("Отвергнутые объекты Астаны — тоже НЕ свалки, но того же города:")
        log.info("   в диапазоне свалок %.0f%%, в диапазоне пашни %.0f%%",
                 100 * inside_dumps, 100 * inside_fields)
        if inside_dumps > inside_fields:
            log.warning("")
            log.warning("ВЫВОД: разделение ЛОЖНОЕ. Не-свалки Астаны похожи на свалки")
            log.warning("Астаны, а не на пашню Алматы. Значит, признак различает")
            log.warning("МЕСТНОСТЬ, а не класс объекта, и шестым признаком не годится.")
            return 0

    log.warning("")
    log.warning("ВЫВОД: разделение есть и не объясняется местностью — стоит")
    log.warning("проверять дальше. При таких размерах групп это ещё не")
    log.warning("доказательство: идеальное разделение выпадает случайно")
    log.warning("примерно в 6%% случаев.")
    return 0
