...
SKELETON = [
    "За свалку платят трижды, и ни один счёт никто не считает.",
...
BLOCKS = [
    (
        "0:00", "0:18", "Проблема",
        [
            "Стихийная свалка — это счёт, который приходит трижды: вывозом, "
            "потерянным вторсырьём и метаном в воздухе.",
            "Вокруг Астаны такие объекты есть, и их нет ни в одном реестре. "
            "Их не считают, потому что не видят: обойти степь пешком "
            "невозможно, а находят по жалобе — когда убирать уже дорого.",
        ],
        "Карта области, на ней несколько синих контуров — то, что размечено "
        "в открытых данных.",
        "Первые две фразы — медленно. Это установка задачи, зритель ещё "
        "настраивается на голос. Счёт, приходящий трижды, — главная мысль "
        "всего ролика: назовите её отчётливо, дальше на неё всё опирается.",
    ),
    (
    ...
    (
        ...
        [
            ...
            "двадцать лет — и отдельно то, что можно вернуть: половину "
            "стоимости уборки закрывает вторсырьё, которое в этой куче лежит.",
    ...
    (
        ...
        [
            ...
            "По ней модель оценивает риск появления новых объектов в следующие "
            "двенадцать месяцев.",
