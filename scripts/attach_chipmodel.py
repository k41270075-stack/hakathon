...
Обучена на казахстанском наборе из OpenStreetMap: 1 165 снимков, из них
745 свалок и 420 трудных отрицательных — карьеры, промплощадки, стройки,
пашня, вода, то есть ровно то, что детектор путает со свалкой.

Кросс-проверка на своих данных 0,899. Перенос на наши объекты **0,786 при
интервале 0,571–0,943**: нижняя граница выше случайного — единственная
модель из трёх обученных, про которую это можно сказать.

Решает состав ОТРИЦАТЕЛЬНЫХ примеров, а не объём. Модель на AerialWaste
училась отличать свалку от ландшафта Ломбардии и дала 0,643 при интервале,
включающем случайное; объединение трёх наборов дало 0,595.

── Чего это всё ещё не значит ──────────────────────────────────────────

Положительные примеры в наборе — ЗАКОННЫЕ полигоны ТБО, крупные и старые.
Наши объекты это стихийные свалки в сотни квадратных метров. Что модель
переносится и на них, показывают семнадцать объектов при трёх
положительных: обнадёживает, но не доказывает.
...
MODEL = Path("models/kz_only_chip.joblib")
