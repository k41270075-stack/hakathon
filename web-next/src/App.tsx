...
const LIMITS: [string, string][] = [
  ...
  [
    ...
    'Детектор ищет место, где растительность исчезла навсегда. В промзоне это редкое событие и потому значимое; в поле — рядовое: залежь, смена оборота, заброшенный огород. Проверено трижды: Алматы 8 находок, восточный пояс Астаны 33, юго-восточный 63 — настоящих свалок ноль. Проверять область научились до прогона: там, где карта подробна, отсев работает, где пуста — нет. Разрыв между работающей областью и провальной оказался четырнадцатикратным.',
  ],
  [
...
export default function App() {
  ...
  const totals = useMemo(() => {
    /* Три числа, а не одно, и это принципиально.
       ...
       «Найдено 49» — то, что программа вынесла на проверку. Свалками
       оказались восемь, а четырнадцать — складами, промплощадками и
      ...
      <main className="mx-auto max-w-[1240px] px-6">
        ...
        <section className="border-t border-grid pt-14 pb-16">
          ...
          <p className="mt-8 max-w-[62ch] text-sm leading-relaxed text-muted-2">
            ...
            OpenStreetMap и человек со снимком 0,5 м на пиксель.
          </p>
        </section>
        ...
        <section className="border-t border-grid pt-14 pb-16">
          ...
          <dl className="mt-9 grid gap-8 sm:grid-cols-3">
            ...
            {[
              ['1 460 км²', 'четыре области вокруг Астаны', 'посчитано полностью'],
