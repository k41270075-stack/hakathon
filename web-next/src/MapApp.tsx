      ...
      {typeof p.highres_score === 'number' && (
        <div className="mt-3 rounded-sm border border-grid bg-soot-2 px-3 py-2.5">
          ...
          <p className="mt-1.5 text-xs leading-snug text-muted-2">
            Обучена на свалках Казахстана из OpenStreetMap: 1 165 снимков,
            отрицательные — карьеры, промплощадки и стройки, то есть то, что
            детектор путает со свалкой. Перенос на наши объекты 0,786 при
            интервале 0,571–0,943. Положительные примеры в обучении —
            законные полигоны, а здесь стихийные свалки: число подсказка, а
            не вердикт.
          </p>
        </div>
      )}
