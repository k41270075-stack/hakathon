...
const visualOf = (p: Props) => {
  if (p.check_source === 'ground') {
    ...
    const when = humanDay(p.ground_date);
...
export default function MapApp() {
  ...
  const [sort, setSort] = useState<SortKey>('damage_p50');
...
function ObjectCard({ f, split }: { f: Feature; split?: Split }) {
  ...
  /* Оценка модели — та же, что в строке списка: highres_score, посчитанная
     ...
     прогоне приходит как null; Number(null) — это 0, не NaN, и в шапке
     карточки появлялось «0%» вместо задуманного прочерка. */
  const model = Number(p.highres_score);
  const hasModel = Number.isFinite(model);
    ...
    <article className="px-5 py-5">
      ...
      {visualOf(p) && (
        <div className="mt-3 rounded-sm border border-grid bg-soot-2 px-3 py-2.5">
          <div className="flex items-baseline justify-between gap-3">
            {/* Подпись зависит от источника: над вердиктом, полученным
       ...
       *
       * Перенос на Казахстан измерен на 51 объекте: ROC-AUC 0,680 при
       * интервале по бутстрэпу 0,517–0,841. Пока объектов было семнадцать,
       * нижняя граница лежала на 0,333 — ниже случайного, и перенос
       * назывался недоказанным. Называть надо нижнюю границу.
       *
      ...
      {typeof p.highres_score === 'number' && (
        <div className="mt-3 rounded-sm border border-grid bg-soot-2 px-3 py-2.5">
          ...
          <p className="mt-1.5 text-xs leading-snug text-muted-2">
            Обучена на AerialWaste, 5 220 снимков. Перенос на Казахстан
            измерен на 51 объекте: <strong className="font-normal text-line">0,680</strong> при
            интервале 0,517–0,841 — нижняя граница выше случайного, но
            выборка мала. На 33 находках, оказавшихся ложными, низкая оценка
            отсеяла бы 88% — но заодно и одну настоящую свалку из восьми.
            Поэтому число здесь подсказка, куда смотреть в последнюю
            очередь, а не фильтр: выброшенный объект не вернётся.
          </p>
        </div>
      )}

      {/* Блок «Вероятность модели» убран.
          Показывалось поле probability, которое заполняет только генератор
          демонстрационных данных. На настоящем прогоне оно приходит как
          null, а Number(null) — это 0, не NaN: задуманный прочерк
          превращался в «0%» прямо под зелёной подписью «проверено».
          Второй блок выше показывает настоящую оценку модели по снимку, и
          двух блоков про одну модель карточке не нужно. */}
