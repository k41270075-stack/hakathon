      ...
      {typeof p.highres_score === 'number' && (
        <div className="mt-3 rounded-sm border border-grid bg-soot-2 px-3 py-2.5">
          ...
          <p className="mt-1.5 text-xs leading-snug text-muted-2">
            Обучена на AerialWaste, 5 220 снимков. Как детектор не доказана:
            перенос 0,643 при интервале 0,333–0,923. На 33 находках,
            оказавшихся ложными, низкая оценка отсеяла бы <strong className="font-normal text-line">88%</strong> —
            но заодно и одну настоящую свалку из восьми. Поэтому число здесь
            подсказка, куда смотреть в последнюю очередь, а не фильтр:
            выброшенный объект не вернётся.
          </p>
        </div>
      )}
        ...
        <p className="mt-1.5 text-xs leading-snug text-muted-2">
          {!hasModel
            ...
            ? (visualOf(p)?.short === 'свалка'
                ? 'Модель возражает: изменение похоже на законное. Но объект уже просмотрен по снимку 0,4–0,8 м, и там видна ссыпка — человек сильнее модели, обученной на чужой стране.'
