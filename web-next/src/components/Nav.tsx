/* Общая навигация поверхностей продукта.
 *
 * Каждая отвечает на свой вопрос, и вопросы заданы разными людьми:
 *   лендинг    — что это и почему этому можно верить;
 *   карта      — куда ехать сегодня (инспектор);
 *   экономика  — что теряется в тенге и сколько вернётся (заказчик);
 ...
 *
 * «Экономика» стоит сразу после карты не по алфавиту: карта показывает
 * работу, а платит за неё тот, кто видит деньги. До этого экрана суммы
 * были размазаны по подписям карточек, и вопрос «а что я получу» не имел
 * одного места, куда на него можно ответить.
 *
...
export type Surface = 'index' | 'map' | 'economy' | 'timelapse' | 'forecast' | 'citizen' | 'label';

const LINKS: [Surface, string, string][] = [
  ['map', './map.html', 'Карта'],
  ['economy', './economy.html', 'Экономика'],
